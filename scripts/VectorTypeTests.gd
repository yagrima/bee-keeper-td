...
func test_vector2_arithmetic() -> bool:
	...
	# Test addition
	var sum = v1 + v2
	if sum != Vector2(150, 275):
		print("❌ Vector2 addition failed")
		return false
	
	# Test subtraction
	var diff = v1 - v2
	if diff != Vector2(50, 125):
		print("❌ Vector2 subtraction failed")
		return false
	
	# Test multiplication
	var mult = v1 * 2.0
	if mult != Vector2(200, 400):
		print("❌ Vector2 multiplication failed")
		return false
	
	# Test division
	var div = v1 / 2.0
	if div != Vector2(50, 100):
		print("❌ Vector2 division failed")
		return false
...
func test_viewport_size_conversion() -> bool:
	...
	if viewport_size != Vector2(1920, 1080):
		print("❌ Vector2i to Vector2 conversion failed")
		return false
	...
	if center_offset != expected:
		print("❌ Viewport size arithmetic failed")
		return false
...
func test_mixed_vector_operations() -> bool:
	...
	var mult = v2 * 1.5
	if mult != Vector2(150, 300):
		print("❌ Vector2 * float failed")
		return false
...
func test_window_centering() -> bool:
	...
	if dialog_position != expected:
		print("❌ Dialog centering failed")
		return false
	...
	if panel_position != expected_panel:
		print("❌ Panel centering failed")
		return false
...
func test_grid_calculations() -> bool:
	...
	if world_pos != Vector2(160, 96):
		print("❌ Grid to world conversion failed")
		return false
	...
	if world_field_pos != expected:
		print("❌ Map offset calculation failed")
		return false
	...
	if button_pos != Vector2(380, 200):
		print("❌ Button positioning failed")
		return false
...
func test_vector_compatibility() -> bool:
...
func test_dialog_size_operations() -> bool:
	...
	if notification_position != Vector2(810, 490):
		print("❌ Notification centering failed")
		return false
...
func test_all_size_properties() -> bool:
	...
	for element in ui_elements:
		...
		if position.x != expected_x:
			print("❌ %s X positioning failed" % element.name)
			return false
		if position.y != expected_y:
			print("❌ %s Y positioning failed" % element.name)
			return false
